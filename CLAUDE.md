...
## Project Overview
**Three Against One** — 3-player hotseat/network turn-based 2.5D strategy board game in Godot 4.6 (~20,500 lines GDScript, 63 scripts).
...
Main (Node2D) — main.gd (~1330 lines, game state machine)
...
│   ├── UnitManager2D (Node2D)          — unit_manager_2d.gd (~3930 lines, units + combat + sprites)
...
**Stage system**: 5 stages × 6 rounds = 30 total. `game_stage_rules.gd` maps round→stage. StageEventService fires `goblin_market_started` event.

**Victory system** (`services/victory_service.gd`): Conquest (town hall HP=0 → winner) + Stage scoring (after round 30 — buildings/units/resources/tech → rank).
...
**Animation**: All sprites rendered via `_draw()` with `draw_texture_rect_region()` on horizontal strip PNGs. Each unit type has constants for: frame count per state (idle/walk/hurt/attack/death), frame size (px), draw size, and duration. Idle is wall-clock driven (`Time.get_ticks_msec()`), other states use tween progress. Recent additions: Orc Bloodaxe (GIF-extracted frames), Elf Worker/Scout/Ranger/Assassin sprite strips.

**Building system**: 16+ types across 8 categories (INFRA, T1_RESOURCE, LORD, GOLD_CHAIN, MILITARY, SCOUT, RECRUIT, SPECIAL). Footprint validation, ghost preview, territory/terrain/AP/resource checks. Upgrade service (level scaling), effect service (passive bonuses), network service (adjacency bonuses). Civilization rules filter per faction+route.

**Tech tree** (`services/technology_service.gd`): Per-faction techs with resource costs, achievement TP gating, unlock effects. `technology_library.gd` defines techs.
...
**Goblin economy**: GoblinMarketUI (card market with reputation pricing) + GoblinHexPanel (stage-based hex selection) + hidden traders + reputation system (0-100).

**Neutral units**: 7 wyverns (guard/aggro), 6 hidden traders, goblin revenge squads. Timer-based combat, dragon blood drops.

**Dragon portal**: Dragon nest mechanics with portal confirm panel. Central volcano feature on the map.
...
**Service**: `network_game_service.gd` (515 lines) manages peer→faction mapping, action request routing (40+ action types), snapshot broadcasting, turn sync. `game_state_serializer.gd` serializes per-faction state. `game_session.gd` (autoload) persists connection state across scene transitions.
...
| 3930 | `unit_manager_2d.gd` | Units, combat, sprite animation, warbands, action preview |
| 1552 | `building_manager_2d.gd` | Building placement, upgrades, networks, effects, recruitment |
| 1443 | `grid_manager_2d.gd` | 6-phase terrain generation, editor painting |
| 1330 | `main.gd` | Game state machine, initializes ALL subsystems |
| 1069 | `neutral/neutral_unit_manager_2d.gd` | Wyvern AI, goblin market, reputation |
| 638 | `templates/default_template_library.gd` | Unit/building template factory |
| 542 | `building_data.gd` | Building factory: 16+ types across 8 categories |
| 515 | `services/network_game_service.gd` | ENet multiplayer: host/join, RPC routing, snapshots |
| 506 | `ui/technology_tree_panel.gd` | Tech tree UI (_draw-based) |
| 471 | `ui/achievement_tree_panel.gd` | Achievement tree UI (_draw-based) |
| 462 | `wall_blueprint_manager_2d.gd` | Dwarf wall planning system |
| 434 | `resource_manager_2d.gd` | 18 resource types, deterministic placement |
| 430 | `ui/civilization_route_panel.gd` | Civilization route selection UI |
| 395 | `unit_info_panel.gd` | Unit details panel with warband UI |
| 333 | `building_ui.gd` | Right-side building cards, category sidebar |
| 324 | `services/achievement_service.gd` | Achievement tracking + TP rewards |
| 307 | `fog_of_war_2d.gd` | Per-player float fog, 3×3 smoothing, 0.3s fade |
| 294 | `services/goblin_hex_service.gd` | Goblin hex card logic |
| 272 | `ui/goblin_market_ui.gd` | Card market overlay |
| 260 | `services/recruitment_service.gd` | Queue-based recruitment |
| 258 | `services/technology_service.gd` | Tech tree state + research |
